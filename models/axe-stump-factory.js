 ...
 * -------------------------------------------------
 * O MACHADO NO TOCO DO QUINTAL LATERAL
 *
 * Terceira das tres pecas do pacote de madeira (psx-wood-pack): o
 * machado cravado no toco. Ver models/woodpile-factory.js para a
 * documentacao completa do pacote.
 *
 * O machado sai junto com o toco de proposito: no arquivo original os dois
 * sao UMA casca fechada, fundida na malha (o README do pacote explica),
 * entao separar exigiria cortar geometria e abrir buracos - o que mudaria o
 * modelo. Fica como uma peca so, do lado da pilha de lenha.
 *
...
window.AxeStumpFactory = (function () {
  const MODEL_URL = "assets/models/axe_stump_psx.glb";

  // Nome legivel dado as malhas de dentro do arquivo. O .glb chama a peca
  // de "machado"; renomear na hora de carregar deixa a arvore do
  ...
  const NATIVE_MIN_X = -0.13033747673034668;
  const NATIVE_MAX_X = 0.13033747673034668;
  const NATIVE_MIN_Y = 0.0;
  const NATIVE_MAX_Y = 0.2609364092350006;
  const NATIVE_MIN_Z = -0.1051066443324089;
  const NATIVE_MAX_Z = 0.1051066443324089;
  ...
  // 0.782 x 0.783 x 0.631 m - medida de um toco de cortar lenha com o machado cravado.
  const MODEL_SCALE = 3;
  ...
  function loadInto(group, state) {
    getLoader().load(
      ...
      function onError(error) {
        console.error("AxeStumpFactory: falha ao carregar " + MODEL_URL, error);
      }
    );
  }

  function createAxeStump() {
  ...
  return { createAxeStump: createAxeStump };
})();
