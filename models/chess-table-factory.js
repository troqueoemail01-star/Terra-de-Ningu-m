/**
 * models/chess-table-factory.js
 * -------------------------------------------------
 * Mesa de xadrez - peca decorativa de "MEU QUARTO"
 *
 * Mesa de xadrez de madeira com o tabuleiro e as pecas em cima, tudo numa malha unica. O arquivo ja chega na escala real (0.78 x 0.68 x 0.71 m: altura de mesa de jogo baixa), por isso MODEL_SCALE = 1. Traz COLOR_0 (cor por vertice) alem da textura - o GLTFLoader liga vertexColors sozinho, nada a fazer aqui.
 *
 ...
 * O pacote psx-chess-table veio com preview proprio e um runtime PSX em modulo
...
window.ChessTableFactory = (function () {
  const MODEL_URL = "assets/models/chess_table_psx.glb";
  ...
  const MESH_NAME = "chess_table_psx";
  ...
  // tamanho dela.
  const NATIVE_MIN_X = -0.389535487;
  const NATIVE_MAX_X = 0.389535487;
  const NATIVE_MIN_Y = 0.000000000;
  const NATIVE_MAX_Y = 0.681473076;
  const NATIVE_MIN_Z = -0.357002854;
  const NATIVE_MAX_Z = 0.357002854;
  ...
  // verdade (0.779 x 0.681 x 0.714 m), entao nao precisa
  ...
  function createChessTable() {
    ...
    getLoader().load(
      ...
      function onError(error) {
        console.error("ChessTableFactory: falha ao carregar " + MODEL_URL, error);
      }
    );
  ...
  return { createChessTable: createChessTable };
})();
