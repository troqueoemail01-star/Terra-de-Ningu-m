 ...
 * -------------------------------------------------
 * A GALINHA PORTA-FERRAMENTAS DO QUINTAL LATERAL
 *
 * Terceira das tres pecas do pacote de jardim (psx-garden-pack): a
 * galinha de ceramica com as ferramentas de jardim dentro. Ver
 * models/dirt-mound-factory.js para a documentacao completa do pacote.
 *
 * A cabeca da galinha aponta para -X no arquivo, entao os dados dela em
 * scenes/corridor-config.js pedem rotationY: Math.PI - e o que faz ela
 * olhar para o quintal, e nao para a parede da casa. Uma linha de dado, no
 * arquivo de dados: o Editor gira ela por cima disso sem tocar em codigo.
 *
...
window.ChickenToolPotFactory = (function () {
  const MODEL_URL = "assets/models/chicken_toolpot_psx.glb";

  // Nome legivel dado as malhas de dentro do arquivo. O .glb chama a peca
  // de "chicken_toolpot"; renomear na hora de carregar deixa a arvore do
  ...
  const MESH_NAME = "chicken_toolpot_psx";
  ...
  const NATIVE_MIN_X = -0.13024654984474182;
  const NATIVE_MAX_X = 0.13024654984474182;
  const NATIVE_MIN_Y = 1.0621694314094654e-06;
  const NATIVE_MAX_Y = 0.25847489713156346;
  const NATIVE_MIN_Z = -0.11886632363527816;
  const NATIVE_MAX_Z = 0.11886632363527816;
  ...
  // as medidas do arquivo batem com a peca de verdade - 0.260 x 0.258 x 0.238 m,
  // ou seja uma galinha de ceramica de enfeite, com ferramentas de jardim dentro.
  const MODEL_SCALE = 1;
  ...
  function loadInto(group, state) {
    getLoader().load(
      ...
      function onError(error) {
        console.error("ChickenToolPotFactory: falha ao carregar " + MODEL_URL, error);
      }
    );
  }

  function createChickenToolPot() {
  ...
  return { createChickenToolPot: createChickenToolPot };
})();
