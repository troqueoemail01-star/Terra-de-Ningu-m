/**
 * models/dirt-mound-factory.js
 * -------------------------------------------------
 * O MONTINHO DE TERRA DO QUINTAL LATERAL
 *
 * Primeira das TRES pecas do pacote de jardim (psx-garden-pack) que o
 * jogador enviou: o montinho de terra. As outras duas sao o vaso de
 * samambaia (models/fern-pot-factory.js) e a galinha porta-ferramentas
 * (models/chicken-toolpot-factory.js).
 *
 * ---------- Tres pecas, tres modelos INDIVIDUAIS ----------
 * O pacote traz as tres juntas num garden_pack.glb e tambem cada uma no
 * seu arquivo. Foram os arquivos INDIVIDUAIS que entraram, um por peca -
 * exatamente o pedido ("devem ser adicionados como modelos individuais").
 *
 * ---------- Geometria quantizada (KHR_mesh_quantization) ----------
 * A malha deste pacote vem com as posicoes/UVs em inteiros de 16 bits e a
 * escala real no proprio no do arquivo (a extensao KHR_mesh_quantization,
 * marcada como required). Nao precisou de NADA novo: o GLTFLoader do
 * three.js r128 le essa extensao nativamente, e o jogo ja carrega um
 * modelo assim desde a cadeira do MEU QUARTO (chair_psx.glb). Por isso os
 * NATIVE_* abaixo sao as medidas JA com o no resolvido - nao os inteiros
 * crus do accessor.
 *
...
window.DirtMoundFactory = (function () {
  const MODEL_URL = "assets/models/dirt_mound_psx.glb";

  // Nome legivel dado as malhas de dentro do arquivo. O .glb chama a peca
  // de "dirt_mound"; renomear na hora de carregar deixa a arvore do
  ...
  const MESH_NAME = "dirt_mound_psx";
  ...
  const NATIVE_MIN_X = -0.19998028874397278;
  const NATIVE_MAX_X = 0.19998028874397278;
  const NATIVE_MIN_Y = 7.497477848630862e-07;
  const NATIVE_MAX_Y = 0.12246556111094165;
  const NATIVE_MIN_Z = -0.172028089200304;
  const NATIVE_MAX_Z = 0.172028089200304;
  ...
  // as medidas do arquivo batem com a peca de verdade - 0.400 x 0.122 x 0.344 m,
  // ou seja um montinho de terra revirada.
  const MODEL_SCALE = 1;
  ...
  function loadInto(group, state) {
    getLoader().load(
      ...
      function onError(error) {
        console.error("DirtMoundFactory: falha ao carregar " + MODEL_URL, error);
      }
    );
  }
  ...
  return { createDirtMound: createDirtMound };
})();
