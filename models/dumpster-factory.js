/**
 * models/dumpster-factory.js
 * -------------------------------------------------
 * A LIXEIRA GRANDE (dumpster) DO QUINTAL LATERAL
 *
 * Primeira das CINCO pecas do pacote de lixo (psx-trash-pack) que o
 * jogador enviou: o caixote verde de lixo. As outras quatro sao a lata de
 * lixo (models/yard-trash-can-factory.js) e os tres sacos
 * (models/trash-bag-a-factory.js, -b- e -c-).
 *
 * ---------- Cinco pecas, cinco modelos INDIVIDUAIS ----------
 * O pacote traz as cinco pecas juntas num .glb unico (trash_pack_full.glb,
 * com os cinco nos separados) e tambem cada uma no seu proprio arquivo.
 * Foram os arquivos INDIVIDUAIS que entraram, um por peca, cada um com sua
 * fabrica e seu grupo de nome proprio - exatamente o pedido: "devem ser
 * adicionados como modelos individuais, assim o player pode modificar cada
 * um individualmente, no editor". No Editor cada peca aparece na lista com
 * nome proprio e guarda posicao, rotacao e escala dela sozinha.
 *
...
window.DumpsterFactory = (function () {
  const MODEL_URL = "assets/models/dumpster_psx.glb";

  // Nome legivel dado as malhas de dentro do arquivo. O .glb chama a peca
  // de "dumpster"; renomear na hora de carregar deixa a arvore do
  ...
  const NATIVE_MIN_X = -0.2526665925979614;
  const NATIVE_MAX_X = 0.2526665925979614;
  const NATIVE_MIN_Y = 0.0;
  const NATIVE_MAX_Y = 0.3804316520690918;
  const NATIVE_MIN_Z = -0.14017339050769806;
  const NATIVE_MAX_Z = 0.14017339050769806;
  const NATIVE_CENTER_X = (NATIVE_MIN_X + NATIVE_MAX_X) / 2;
  const NATIVE_CENTER_Z = (NATIVE_MIN_Z + NATIVE_MAX_Z) / 2;

  // ---------- Escala ----------
  // O pacote veio nas unidades do arquivo original, e o LEIA-ME dele diz
  // com todas as letras qual e o fator para metros: PSX_PACK.unitScaleHint
  // = 3.2 ("dumpster = 0,50 de largura ... multiplique por ~3,2 pra ficar
  // em metros"). Com ele a peca sai em 1.617 x 1.217 x 0.897 m - medida de
  // caixote de lixo de rua. Esticar so um eixo nunca esta em jogo:
  // distorceria a peca e a textura.
  ...
  function loadInto(group, state) {
    getLoader().load(
      ...
      function onError(error) {
        console.error("DumpsterFactory: falha ao carregar " + MODEL_URL, error);
      }
    );
  }
  ...
  return { createDumpster: createDumpster };
})();
