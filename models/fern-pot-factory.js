/**
 * models/fern-pot-factory.js
 * -------------------------------------------------
 * O VASO DE SAMAMBAIA DO QUINTAL LATERAL
 *
 * Segunda das tres pecas do pacote de jardim (psx-garden-pack): o vaso
 * com a samambaia. Ver models/dirt-mound-factory.js para a documentacao
 * completa do pacote, da geometria quantizada e do caminho de import.
 *
 * O material do arquivo vem doubleSided, e a peca DEPENDE disso: as folhas
 * sao cartoes abertos: sem as duas faces, metade da folhagem sumiria
 * dependendo do angulo. O side do arquivo e preservado nas duas versoes
 * de material (noite e dia).
 *
...
window.FernPotFactory = (function () {
  const MODEL_URL = "assets/models/fern_pot_psx.glb";

  // Nome legivel dado as malhas de dentro do arquivo. O .glb chama a peca
  // de "fern_pot"; renomear na hora de carregar deixa a arvore do
  ...
  const NATIVE_MIN_X = -0.20675194263458252;
  const NATIVE_MAX_X = 0.20675194263458252;
  const NATIVE_MIN_Y = 1.0223919215401889e-07;
  const NATIVE_MAX_Y = 0.2414746603197449;
  const NATIVE_MIN_Z = -0.1805285754178912;
  const NATIVE_MAX_Z = 0.1805285754178912;
  ...
  // as medidas do arquivo batem com a peca de verdade - 0.414 x 0.241 x 0.361 m,
  // ou seja um vaso baixo e largo com samambaia.
  const MODEL_SCALE = 1;
  ...
  function loadInto(group, state) {
    getLoader().load(
      ...
      function onError(error) {
        console.error("FernPotFactory: falha ao carregar " + MODEL_URL, error);
      }
    );
  }

  function createFernPot() {
  ...
  return { createFernPot: createFernPot };
})();
