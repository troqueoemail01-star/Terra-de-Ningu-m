/**
 * models/plant-bed-factory.js
 * -------------------------------------------------
 * Canteiro de plantas - peca decorativa de "MEU QUARTO"
 *
 * CUIDADO: nao confundir com models/flower-bed-factory.js, que sao os canteiros de rosas do quintal da frente. Este e o canteiro comprido de plantas e flores do pacote psx_plantas, peca de DENTRO do quarto. O material vem doubleSided e a peca DEPENDE disso: as folhas sao cartoes abertos.
 *
 ...
 * O pacote psx_plantas veio com preview proprio e um runtime PSX em modulo
...
window.PlantBedFactory = (function () {
  const MODEL_URL = "assets/models/plant_bed_psx.glb";
  ...
  // tamanho dela.
  const NATIVE_MIN_X = -0.494682274;
  const NATIVE_MAX_X = 0.494682274;
  const NATIVE_MIN_Y = 0.000000000;
  const NATIVE_MAX_Y = 0.697846589;
  const NATIVE_MIN_Z = -0.220087756;
  const NATIVE_MAX_Z = 0.220087756;
  ...
  // verdade (0.989 x 0.698 x 0.440 m), entao nao precisa
  ...
  function createPlantBed() {
    ...
    getLoader().load(
      ...
      function onError(error) {
        console.error("PlantBedFactory: falha ao carregar " + MODEL_URL, error);
      }
    );
  ...
  return { createPlantBed: createPlantBed };
})();
