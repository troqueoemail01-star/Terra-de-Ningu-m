/**
 * models/power-meter-factory.js
 * -------------------------------------------------
 * O MEDIDOR DE ENERGIA DA PAREDE LATERAL DIREITA
 *
 * O quarto pacote que o jogador enviou (medidor_energia_psx) e a UNICA
 * peca desta rodada que fica PENDURADA NA PAREDE, e nao apoiada no chao: o
 * quadro do medidor de energia, com a caixa, a porta, o mostrador e os
 * quatro cabos. O LEIA-ME do pacote confirma que a malha e uma peca unica
 * (caixa, porta, medidor e cabos formam a mesma superficie), entao aqui nao
 * havia nada para separar - diferente dos outros tres pacotes.
 *
 * Fica em Y = 1.05 do chao (campo elevation dos dados, o mesmo que a
 * espelheira e a toalha do BANHEIRO usam), com a frente virada para o
 * quintal: o mostrador cai na altura dos olhos, como num medidor de
 * verdade. E a unica peca da lista com solid: false - a colisao do jogo e
 * um AABB sem eixo Y, entao um solido num quadro pendurado a 1 m do chao
 * viraria parede invisivel para quem passa por baixo dele.
 *
 * ---------- AO assada em COLOR_0 ----------
 * Este e o primeiro modelo do jogo com sombra de contato assada nas CORES
 * DE VERTICE (COLOR_0). O GLTFLoader liga vertexColors sozinho ao ler o
 * arquivo; o que esta fabrica faz de diferente e PRESERVAR essa flag nas
 * duas versoes de material (a de noite, que troca o Unlit do arquivo por
 * MeshStandard, e a chapada de dia) - sem isso a peca perderia a AO e
 * ficaria lisa.
 *
...
window.PowerMeterFactory = (function () {
  const MODEL_URL = "assets/models/power_meter_psx.glb";

  // Nome legivel dado as malhas de dentro do arquivo. O .glb chama a peca
  // de "MedidorEnergia"; renomear na hora de carregar deixa a arvore do
  ...
  const MESH_NAME = "power_meter_psx";
  ...
  const NATIVE_MIN_X = -0.3830108940601349;
  const NATIVE_MAX_X = 0.3830108940601349;
  const NATIVE_MIN_Y = 0.0;
  const NATIVE_MAX_Y = 0.8575339913368225;
  const NATIVE_MIN_Z = -0.29725751280784607;
  const NATIVE_MAX_Z = 0.29725751280784607;
  const NATIVE_CENTER_X = (NATIVE_MIN_X + NATIVE_MAX_X) / 2;
  const NATIVE_CENTER_Z = (NATIVE_MIN_Z + NATIVE_MAX_Z) / 2;

  // ---------- Escala ----------
  // Sem reescala (1): o manifesto do pacote declara "1 unit = 1 metre" e
  // as medidas do arquivo batem com a peca de verdade - 0.766 x 0.858 x 0.595 m,
  // ou seja um quadro de medidor de energia com os quatro cabos.
  const MODEL_SCALE = 1;
  ...
  // de models/microwave-factory.js, de models/portable-radio-factory.js e
  // de models/toilet-factory.js, com os mesmos numeros de acabamento. Sem
  // isto a peca brilharia sozinha no meio da noite, ignorando as luzes da
  // cena (KHR_materials_unlit vira MeshBasicMaterial no GLTFLoader).
  //
  ...
  //
  // vertexColors e repassado: este arquivo traz AO assada em COLOR_0 (ver
  // o topo), e o MeshStandard novo tem que continuar lendo essa cor de
  // vertice - sem isso a sombra de contato do medidor desapareceria.
  ...
  // ligada.
  // vertexColors segue ligado: a AO assada em COLOR_0 vale de dia
  // tambem (ver o topo).
  ...
  function loadInto(group, state) {
    getLoader().load(
      ...
      function onError(error) {
        console.error("PowerMeterFactory: falha ao carregar " + MODEL_URL, error);
      }
    );
  }
  ...
  return { createPowerMeter: createPowerMeter };
})();
