/**
 * models/rocking-chair-factory.js
 * -------------------------------------------------
 * Cadeira de balanco - peca decorativa de "MEU QUARTO"
 *
 * Cadeira de balanco de madeira. Entrou a malha OTIMIZADA do pacote (rocking_chair_opt.glb, 34.594 tris / 14.499 verts em vez de 50.000 tris / 56.599 verts) - foi a propria recomendacao do README do pacote para uso em jogo, mesma silhueta e 1,3 MB menos. Sem animacao: a cadeira nao balanca (peca decorativa).
 *
 ...
 * O pacote rocking_chair_psx_threejs veio com preview proprio e um runtime PSX em modulo
...
window.RockingChairFactory = (function () {
  const MODEL_URL = "assets/models/rocking_chair_psx.glb";
  ...
  const MESH_NAME = "rocking_chair_psx";
  ...
  // tamanho dela.
  const NATIVE_MIN_X = -0.251799643;
  const NATIVE_MAX_X = 0.251592398;
  const NATIVE_MIN_Y = 0.000000000;
  const NATIVE_MAX_Y = 0.848658025;
  const NATIVE_MIN_Z = -0.415417552;
  const NATIVE_MAX_Z = 0.415417552;
  ...
  // ---------- Escala ----------
  // O arquivo nao chega no tamanho de uso (0.503 x 0.849 x 0.831 m nativos),
  // entao a peca e normalizada pela ALTURA, mesma ideia de
  // TARGET_HEIGHT/MODEL_SCALE de BookshelfFactory/TrashCanFactory/
  // TrophyFactory. Resultado final: 0.593 x 1.000 x 0.979 m.
  const TARGET_HEIGHT = 1.00;
  ...
  function createRockingChair() {
    ...
    getLoader().load(
      ...
      function onError(error) {
        console.error("RockingChairFactory: falha ao carregar " + MODEL_URL, error);
      }
    );
  ...
  return { createRockingChair: createRockingChair };
})();
