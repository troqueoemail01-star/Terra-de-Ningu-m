/**
 * models/round-pot-factory.js
 * -------------------------------------------------
 * Vaso oval de planta - peca decorativa de "MEU QUARTO"
 *
 * Segunda peca do pacote psx_plantas: a bacia/vaso oval que estava solta do lado do canteiro. CUIDADO: nao confundir com "Vaso de planta" (models/floor-plant-factory.js, o vaso que ja estava na parede esquerda) nem com "Vaso de samambaia" (models/fern-pot-factory.js, do quintal).
 *
...
window.RoundPotFactory = (function () {
  const MODEL_URL = "assets/models/round_pot_psx.glb";
  ...
  // tamanho dela.
  const NATIVE_MIN_X = -0.193376464;
  const NATIVE_MAX_X = 0.193376464;
  const NATIVE_MIN_Y = 0.000000000;
  const NATIVE_MAX_Y = 0.549432119;
  const NATIVE_MIN_Z = -0.095357304;
  const NATIVE_MAX_Z = 0.095357304;
  ...
  // verdade (0.387 x 0.549 x 0.191 m), entao nao precisa
  ...
  function createRoundPot() {
    ...
    getLoader().load(
      ...
      function onError(error) {
        console.error("RoundPotFactory: falha ao carregar " + MODEL_URL, error);
      }
    );
  ...
  return { createRoundPot: createRoundPot };
})();
