/**
 * models/side-table-factory.js
 * -------------------------------------------------
 * Mesa de canto - peca decorativa de "MEU QUARTO"
 *
 * CUIDADO: nao confundir com o CRIADO-MUDO da cabeceira (models/nightstand-factory.js, assets/models/nightstand_psx.glb). Sao dois moveis diferentes; este entrou como mesa de canto/apoio para nao repetir o nome do que ja existia. Malha leve (1.490 tris).
 *
 ...
 * O pacote psx_night_table veio com preview proprio e um runtime PSX em modulo
...
window.SideTableFactory = (function () {
  const MODEL_URL = "assets/models/side_table_psx.glb";
  ...
  const MESH_NAME = "side_table_psx";
  ...
  // tamanho dela.
  const NATIVE_MIN_X = -0.302616358;
  const NATIVE_MAX_X = 0.302616358;
  const NATIVE_MIN_Y = 0.000000000;
  const NATIVE_MAX_Y = 0.785772085;
  const NATIVE_MIN_Z = -0.293219924;
  const NATIVE_MAX_Z = 0.293219924;
  ...
  // verdade (0.605 x 0.786 x 0.586 m), entao nao precisa
  ...
  function createSideTable() {
    ...
    getLoader().load(
      ...
      function onError(error) {
        console.error("SideTableFactory: falha ao carregar " + MODEL_URL, error);
      }
    );
  ...
  return { createSideTable: createSideTable };
})();
