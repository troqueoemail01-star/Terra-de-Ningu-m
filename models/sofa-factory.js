/**
 * models/sofa-factory.js
 * -------------------------------------------------
 * Sofa - peca decorativa de "MEU QUARTO"
 *
 * O pacote do sofa NAO trouxe .glb: so o formato proprio .psxm (posicao em ponto fixo 16-bit, normal int8, uv uint16, indice uint16) mais um leitor em modulo ES. Em vez de trazer um SEGUNDO carregador para dentro do jogo, o .psxm foi convertido para .glb no mesmo formato de todos os outros modelos daqui (mesma geometria, mesmos UVs, textura sofa_albedo_256.png embutida) - assim o caminho de import continua sendo UM so: GLTFLoader. Entrou o LOD 1 (8.000 tris), o que o proprio pacote recomenda para jogo. TARGET_HEIGHT poe o sofa em 87 cm de altura (o arquivo nativo tem 51 cm; o manifesto do pacote sugeria ~1.7x).
 *
 ...
 * O pacote psx_sofa veio com preview proprio e um runtime PSX em modulo
...
window.SofaFactory = (function () {
  const MODEL_URL = "assets/models/sofa_psx.glb";
  ...
  // tamanho dela.
  const NATIVE_MIN_X = -0.526942498;
  const NATIVE_MAX_X = 0.526942498;
  const NATIVE_MIN_Y = 0.000000000;
  const NATIVE_MAX_Y = 0.510342857;
  const NATIVE_MIN_Z = -0.224545703;
  const NATIVE_MAX_Z = 0.224545703;
  ...
  // ---------- Escala ----------
  // O arquivo nao chega no tamanho de uso (1.054 x 0.510 x 0.449 m nativos),
  // entao a peca e normalizada pela ALTURA, mesma ideia de
  // TARGET_HEIGHT/MODEL_SCALE de BookshelfFactory/TrashCanFactory/
  // TrophyFactory. Resultado final: 1.797 x 0.870 x 0.766 m.
  const TARGET_HEIGHT = 0.87;
  ...
  function createSofa() {
    ...
    getLoader().load(
      ...
      function onError(error) {
        console.error("SofaFactory: falha ao carregar " + MODEL_URL, error);
      }
    );
