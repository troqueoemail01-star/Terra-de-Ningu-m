/**
 * models/tavern-bottle-01-factory.js
 * -------------------------------------------------
 * Garrafa 1 da taverna (jarro baixo) - peca decorativa de "MEU QUARTO"
 *
 * Uma das cinco garrafas que vinham COLADAS na mesa de taverna (ver models/tavern-table-factory.js): jarro baixo e largo. Cada garrafa virou um .glb e uma fabrica separada, para o Editor pegar uma sem levar as outras. Diferente da mesa, a garrafa entra na escala NATIVA do arquivo (MODEL_SCALE = 1): nessa escala ela mede como garrafa de verdade, e esticar ela 1.75x junto com a mesa daria uma garrafa de 60 cm.
 *
...
window.TavernBottle01Factory = (function () {
  const MODEL_URL = "assets/models/tavern_bottle_01_psx.glb";
  ...
  const MESH_NAME = "tavern_bottle_01_psx";
  ...
  // tamanho dela.
  const NATIVE_MIN_X = -0.062773667;
  const NATIVE_MAX_X = 0.062773667;
  const NATIVE_MIN_Y = 0.000000000;
  const NATIVE_MAX_Y = 0.247768909;
  const NATIVE_MIN_Z = -0.061214723;
  const NATIVE_MAX_Z = 0.061214723;
  ...
  // verdade (0.126 x 0.248 x 0.122 m), entao nao precisa
  ...
  function createTavernBottle01() {
    ...
    getLoader().load(
      ...
      function onError(error) {
        console.error("TavernBottle01Factory: falha ao carregar " + MODEL_URL, error);
      }
    );
  ...
  return { createTavernBottle01: createTavernBottle01 };
})();
