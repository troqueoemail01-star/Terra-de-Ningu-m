/**
 * models/tavern-bottle-02-factory.js
 * -------------------------------------------------
 * Garrafa 2 da taverna (garrafao) - peca decorativa de "MEU QUARTO"
 *
 * Uma das cinco garrafas que vinham COLADAS na mesa de taverna (ver models/tavern-table-factory.js): garrafao / decanter arredondado. Cada garrafa virou um .glb e uma fabrica separada, para o Editor pegar uma sem levar as outras. Diferente da mesa, a garrafa entra na escala NATIVA do arquivo (MODEL_SCALE = 1): nessa escala ela mede como garrafa de verdade, e esticar ela 1.75x junto com a mesa daria uma garrafa de 60 cm.
 *
...
window.TavernBottle02Factory = (function () {
  const MODEL_URL = "assets/models/tavern_bottle_02_psx.glb";
  ...
  const MESH_NAME = "tavern_bottle_02_psx";
  ...
  // tamanho dela.
  const NATIVE_MIN_X = -0.080441758;
  const NATIVE_MAX_X = 0.080441758;
  const NATIVE_MIN_Y = 0.000000000;
  const NATIVE_MAX_Y = 0.341929409;
  const NATIVE_MIN_Z = -0.076284561;
  const NATIVE_MAX_Z = 0.076284561;
  ...
  // verdade (0.161 x 0.342 x 0.153 m), entao nao precisa
  ...
  function createTavernBottle02() {
    ...
    getLoader().load(
      ...
      function onError(error) {
        console.error("TavernBottle02Factory: falha ao carregar " + MODEL_URL, error);
      }
    );
  ...
  return { createTavernBottle02: createTavernBottle02 };
})();
