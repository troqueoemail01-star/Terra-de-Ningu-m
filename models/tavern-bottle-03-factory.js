/**
 * models/tavern-bottle-03-factory.js
 * -------------------------------------------------
 * Garrafa 3 da taverna (quadrada) - peca decorativa de "MEU QUARTO"
 *
 * Uma das cinco garrafas que vinham COLADAS na mesa de taverna (ver models/tavern-table-factory.js): garrafa quadrada de gargalo curto. Cada garrafa virou um .glb e uma fabrica separada, para o Editor pegar uma sem levar as outras. Diferente da mesa, a garrafa entra na escala NATIVA do arquivo (MODEL_SCALE = 1): nessa escala ela mede como garrafa de verdade, e esticar ela 1.75x junto com a mesa daria uma garrafa de 60 cm.
 *
...
window.TavernBottle03Factory = (function () {
  const MODEL_URL = "assets/models/tavern_bottle_03_psx.glb";
  ...
  const MESH_NAME = "tavern_bottle_03_psx";
  ...
  // tamanho dela.
  const NATIVE_MIN_X = -0.064020813;
  const NATIVE_MAX_X = 0.064020813;
  const NATIVE_MIN_Y = 0.000000000;
  const NATIVE_MAX_Y = 0.252549681;
  const NATIVE_MIN_Z = -0.062461875;
  const NATIVE_MAX_Z = 0.062461875;
  ...
  // ---------- Escala ----------
  // Sem reescala (1): as medidas do arquivo ja batem com a peca de
  // verdade (0.128 x 0.253 x 0.125 m), entao nao precisa
  // de nenhum fator do tipo TARGET_HEIGHT.
  const MODEL_SCALE = 1;
  ...
  function createTavernBottle03() {
    ...
    getLoader().load(
      ...
      function onError(error) {
        console.error("TavernBottle03Factory: falha ao carregar " + MODEL_URL, error);
      }
    );
  ...
  return { createTavernBottle03: createTavernBottle03 };
})();
