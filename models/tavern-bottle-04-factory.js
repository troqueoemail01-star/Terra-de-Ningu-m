/**
 * models/tavern-bottle-04-factory.js
 * -------------------------------------------------
 * Garrafa 4 da taverna (alta) - peca decorativa de "MEU QUARTO"
 *
 * Uma das cinco garrafas que vinham COLADAS na mesa de taverna (ver models/tavern-table-factory.js): garrafa alta classica. Cada garrafa virou um .glb e uma fabrica separada, para o Editor pegar uma sem levar as outras. Diferente da mesa, a garrafa entra na escala NATIVA do arquivo (MODEL_SCALE = 1): nessa escala ela mede como garrafa de verdade, e esticar ela 1.75x junto com a mesa daria uma garrafa de 60 cm.
 *
...
window.TavernBottle04Factory = (function () {
  const MODEL_URL = "assets/models/tavern_bottle_04_psx.glb";
  ...
  const MESH_NAME = "tavern_bottle_04_psx";
  ...
  // tamanho dela.
  const NATIVE_MIN_X = -0.060071483;
  const NATIVE_MAX_X = 0.060071483;
  const NATIVE_MIN_Y = 0.000000000;
  const NATIVE_MAX_Y = 0.363338977;
  const NATIVE_MIN_Z = -0.063293315;
  const NATIVE_MAX_Z = 0.063293315;
  ...
  // verdade (0.120 x 0.363 x 0.127 m), entao nao precisa
  ...
  function createTavernBottle04() {
    ...
    getLoader().load(
      ...
      function onError(error) {
        console.error("TavernBottle04Factory: falha ao carregar " + MODEL_URL, error);
      }
    );
  ...
  return { createTavernBottle04: createTavernBottle04 };
})();
