/**
 * models/tavern-bottle-05-factory.js
 * -------------------------------------------------
 * Garrafa 5 da taverna (frasco) - peca decorativa de "MEU QUARTO"
 *
 * Uma das cinco garrafas que vinham COLADAS na mesa de taverna (ver models/tavern-table-factory.js): frasco alto quadrado. Cada garrafa virou um .glb e uma fabrica separada, para o Editor pegar uma sem levar as outras. Diferente da mesa, a garrafa entra na escala NATIVA do arquivo (MODEL_SCALE = 1): nessa escala ela mede como garrafa de verdade, e esticar ela 1.75x junto com a mesa daria uma garrafa de 60 cm.
 *
...
window.TavernBottle05Factory = (function () {
  const MODEL_URL = "assets/models/tavern_bottle_05_psx.glb";
  ...
  const MESH_NAME = "tavern_bottle_05_psx";
  ...
  // tamanho dela.
  const NATIVE_MIN_X = -0.054667115;
  const NATIVE_MAX_X = 0.054667115;
  const NATIVE_MIN_Y = 0.000000000;
  const NATIVE_MAX_Y = 0.277492851;
  const NATIVE_MIN_Z = -0.057057522;
  const NATIVE_MAX_Z = 0.057057522;
  ...
  // verdade (0.109 x 0.277 x 0.114 m), entao nao precisa
  ...
  function createTavernBottle05() {
    ...
    getLoader().load(
      ...
      function onError(error) {
        console.error("TavernBottle05Factory: falha ao carregar " + MODEL_URL, error);
      }
    );
  ...
  return { createTavernBottle05: createTavernBottle05 };
})();
