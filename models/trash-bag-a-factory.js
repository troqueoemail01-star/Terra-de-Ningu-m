/**
 * models/trash-bag-a-factory.js
 * -------------------------------------------------
 * O SACO DE LIXO A DO QUINTAL LATERAL
 *
 * Uma das cinco pecas do pacote de lixo (psx-trash-pack): o saco de lixo
 * A. Os tres sacos (A, B e C) sao malhas DIFERENTES uma da outra - o
...
window.TrashBagAFactory = (function () {
  const MODEL_URL = "assets/models/trash_bag_a_psx.glb";

  // Nome legivel dado as malhas de dentro do arquivo. O .glb chama a peca
  // de "trash_bag_a"; renomear na hora de carregar deixa a arvore do
  ...
  const MESH_NAME = "trash_bag_a_psx";
  ...
  const NATIVE_MIN_X = -0.10526639223098755;
  const NATIVE_MAX_X = 0.10526639223098755;
  const NATIVE_MIN_Y = 0.0;
  const NATIVE_MAX_Y = 0.232076957821846;
  const NATIVE_MIN_Z = -0.11249323934316635;
  const NATIVE_MAX_Z = 0.11249323934316635;
  ...
  // em metros"). Com ele a peca sai em 0.674 x 0.743 x 0.720 m - medida de
  ...
  function loadInto(group, state) {
    getLoader().load(
      ...
      function onError(error) {
        console.error("TrashBagAFactory: falha ao carregar " + MODEL_URL, error);
      }
    );
  }

  function createTrashBagA() {
  ...
  return { createTrashBagA: createTrashBagA };
})();
