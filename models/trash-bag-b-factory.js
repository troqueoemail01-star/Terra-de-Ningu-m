/**
 * models/trash-bag-b-factory.js
 * -------------------------------------------------
 * O SACO DE LIXO B DO QUINTAL LATERAL
 *
 * Uma das cinco pecas do pacote de lixo (psx-trash-pack): o saco de lixo
 * B. Os tres sacos (A, B e C) sao malhas DIFERENTES uma da outra - o
...
window.TrashBagBFactory = (function () {
  const MODEL_URL = "assets/models/trash_bag_b_psx.glb";

  // Nome legivel dado as malhas de dentro do arquivo. O .glb chama a peca
  // de "trash_bag_b"; renomear na hora de carregar deixa a arvore do
  ...
  const MESH_NAME = "trash_bag_b_psx";
  ...
  const NATIVE_MIN_X = -0.1019938737154007;
  const NATIVE_MAX_X = 0.1019938737154007;
  const NATIVE_MIN_Y = 0.0;
  const NATIVE_MAX_Y = 0.23289509117603302;
  const NATIVE_MIN_Z = -0.11249323189258575;
  const NATIVE_MAX_Z = 0.11249323189258575;
  ...
  // em metros"). Com ele a peca sai em 0.653 x 0.745 x 0.720 m - medida de
  ...
  function loadInto(group, state) {
    getLoader().load(
      ...
      function onError(error) {
        console.error("TrashBagBFactory: falha ao carregar " + MODEL_URL, error);
      }
    );
  }

  function createTrashBagB() {
  ...
  return { createTrashBagB: createTrashBagB };
})();
