/**
 * models/trash-bag-c-factory.js
 * -------------------------------------------------
 * O SACO DE LIXO C DO QUINTAL LATERAL
 *
 * Uma das cinco pecas do pacote de lixo (psx-trash-pack): o saco de lixo
 * C. Os tres sacos (A, B e C) sao malhas DIFERENTES uma da outra - o
 * pacote separou tres sacos distintos da malha original -, por isso cada
 * um tem seu proprio .glb, sua propria fabrica e seu proprio nome no
 * Editor, e nao um mesmo modelo repetido tres vezes. Ver
 * models/dumpster-factory.js para a documentacao completa do pacote e do
 * caminho de import.
 *
...
window.TrashBagCFactory = (function () {
  const MODEL_URL = "assets/models/trash_bag_c_psx.glb";

  // Nome legivel dado as malhas de dentro do arquivo. O .glb chama a peca
  // de "trash_bag_c"; renomear na hora de carregar deixa a arvore do
  ...
  const MESH_NAME = "trash_bag_c_psx";
  ...
  const NATIVE_MIN_X = -0.10240292549133301;
  const NATIVE_MAX_X = 0.10240292549133301;
  const NATIVE_MIN_Y = 0.0;
  const NATIVE_MAX_Y = 0.19635184109210968;
  const NATIVE_MIN_Z = -0.10444825887680054;
  const NATIVE_MAX_Z = 0.10444825887680054;
  ...
  // em metros"). Com ele a peca sai em 0.655 x 0.628 x 0.669 m - medida de
  // saco de lixo cheio, amarrado no no. Esticar so um eixo nunca esta em jogo:
  ...
  function loadInto(group, state) {
    getLoader().load(
      ...
      function onError(error) {
        console.error("TrashBagCFactory: falha ao carregar " + MODEL_URL, error);
      }
    );
  }

  function createTrashBagC() {
  ...
  return { createTrashBagC: createTrashBagC };
})();
