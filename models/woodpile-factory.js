/**
 * models/woodpile-factory.js
 * -------------------------------------------------
 * A PILHA DE LENHA DO QUINTAL LATERAL
 *
 * Primeira das TRES pecas do pacote de madeira (psx-wood-pack) que o
 * jogador enviou: a pilha de toras. As outras duas sao os gravetos
 * (models/branches-factory.js) e o machado no toco
 * (models/axe-stump-factory.js).
 *
 * ---------- Tres pecas, tres modelos INDIVIDUAIS ----------
 * No arquivo original as tres vinham COLADAS numa malha unica; o pacote as
 * separou por componentes conectados e entrega, alem do pack-completo.glb,
 * um .glb por peca com pivo proprio. Foram os TRES arquivos individuais
 * que entraram, um por fabrica - exatamente o pedido ("devem ser
 * adicionados como modelos individuais").
 *
 * As toras correm no eixo Z do arquivo, entao com rotationY: 0 a pilha ja
 * nasce deitada ao longo da parede lateral (que tambem corre em Z) - e por
 * isso que os dados dela em scenes/corridor-config.js nao pedem giro.
 *
...
window.WoodpileFactory = (function () {
  const MODEL_URL = "assets/models/woodpile_psx.glb";

  // Nome legivel dado as malhas de dentro do arquivo. O .glb chama a peca
  // de "madeira"; renomear na hora de carregar deixa a arvore do
  ...
  const NATIVE_MIN_X = -0.14341044425964355;
  const NATIVE_MAX_X = 0.14341044425964355;
  const NATIVE_MIN_Y = 0.0;
  const NATIVE_MAX_Y = 0.19034239649772644;
  const NATIVE_MIN_Z = -0.2672114372253418;
  const NATIVE_MAX_Z = 0.2672114372253418;
  ...
  // 0.860 x 0.571 x 1.603 m - medida de uma pilha de lenha encostada na parede.
  const MODEL_SCALE = 3;
  ...
  function loadInto(group, state) {
    getLoader().load(
      ...
      function onError(error) {
        console.error("WoodpileFactory: falha ao carregar " + MODEL_URL, error);
      }
    );
  }
  ...
  return { createWoodpile: createWoodpile };
})();
