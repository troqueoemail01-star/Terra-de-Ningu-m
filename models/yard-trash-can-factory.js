/**
 * models/yard-trash-can-factory.js
 * -------------------------------------------------
 * A LATA DE LIXO DO QUINTAL LATERAL
 *
 * Segunda das cinco pecas do pacote de lixo (psx-trash-pack): a lata
 * plastica com tampa. Ver models/dumpster-factory.js para a documentacao
 * completa do pacote e do caminho de import.
 *
 * CUIDADO com o nome: TrashCanFactory (models/trash-can-factory.js) e
 * OUTRA peca - a lixeirinha de metal que fica DENTRO do MEU QUARTO, feita
 * em codigo. Esta e a lata do quintal, modelo importado, e por isso ela se
 * chama YardTrashCanFactory e o arquivo dela em assets/models e
 * yard_trash_can_psx.glb, sem tocar no trash_can_psx.glb que ja existia.
 *
...
window.YardTrashCanFactory = (function () {
  const MODEL_URL = "assets/models/yard_trash_can_psx.glb";

  // Nome legivel dado as malhas de dentro do arquivo. O .glb chama a peca
  // de "trash_can"; renomear na hora de carregar deixa a arvore do
  ...
  const MESH_NAME = "yard_trash_can_psx";
  ...
  const NATIVE_MIN_X = -0.12653784453868866;
  const NATIVE_MAX_X = 0.12653784453868866;
  const NATIVE_MIN_Y = 0.0;
  const NATIVE_MAX_Y = 0.3067997395992279;
  const NATIVE_MIN_Z = -0.12394709140062332;
  const NATIVE_MAX_Z = 0.12394709140062332;
  ...
  // em metros"). Com ele a peca sai em 0.810 x 0.982 x 0.793 m - medida de
  // lata de lixo plastica com tampa. Esticar so um eixo nunca esta em jogo:
  ...
  function loadInto(group, state) {
    getLoader().load(
      ...
      function onError(error) {
        console.error("YardTrashCanFactory: falha ao carregar " + MODEL_URL, error);
      }
    );
  }

  function createYardTrashCan() {
  ...
  return { createYardTrashCan: createYardTrashCan };
})();
